...
# CUSTOM FUNCTIONS
def plot_impulse(t, h_list, label_list=[], markers=True, fig=None, ax=None, ax_labels=True, colors=None):
    '''
        Plots impulse response of several filters with the same time axis. 
        t: time vector, expects numpy array
        h_list: list of numpy arrays containing the impulse response of the filters
        label_list: list of strings with labels to be plot with each filter
    '''
    # if isinstance(h_list, list):
    #     islist = True
    #     if (label_list) and (len(label_list) != len(h_list)):
    #         raise(TypeError, "label_list and h_list must match in length.")
    # elif isinstance(h_list, np.ndarray):
    #     islist = False
    # else:
    #     msg = f"{type(h_list)} is not supported. Either list or np.ndarray "
    #     raise(TypeError, msg)
    islist = False
    ...
    if islist:
        for i, h in enumerate(h_list):
            ax.plot(t, h, line, linewidth=2, label=label_list[i])
    else:
        ax.plot(t, h_list, line, linewidth=2, label=label_list)
    if label_list:
        ax.legend()
    ax.grid(True)
